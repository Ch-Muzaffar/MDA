...
Before opening a pull request, please open an issue and discuss whether the change makes sense in MDA AI. Ensuring a cohesive user experience sometimes means we can't include every possible feature or we need to consider the long-term design of how we want to support a feature area.

- For a high-level overview of how MDA AI works, please see the [Architecture Guide](./docs/architecture.md). Understanding the architecture will help ensure your contributions align with the overall design of the project.
- For a detailed architecture on how the new local agent mode works, please read the [Agent Architecture Guide](./docs/agent_architecture.md).
...
Something that we really appreciate are all the non-code contributions, such as reporting bugs, writing feature requests, and participating in the community.

## Development

MDA AI is an Electron app.
...
MDA AI relies on AI code reviewers to catch issues. If a comment is irrelevant please leave a brief comment and mark the comment as resolved.
